...
class Thim_Ekit_Widget_Login_Popup extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Thim: Login Popup', 'eduma' );
	}
	...
	protected function render() {
		...
		<div class="thim-link-login thim-login-popup">
			...
			if ( is_user_logged_in() ) {
				if ( class_exists( 'LearnPress' ) && ( $profile_text || $show_icon_profile ) ) {
					echo '<a class="profile" href="' . esc_url( learn_press_user_profile_link() ) . '">';
					Icons_Manager::render_icon( $settings['profile_icons'], [ 'aria-hidden' => 'true' ] );
					echo esc_html( $profile_text );
					echo '</a>';
				}
