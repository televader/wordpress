...
class Thim_Ekit_Widget_Countdown_Box extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_control(
			...
			array(
				'label'      => esc_html__( 'Border Radius', 'thim-elementorkits' ),
	...
	protected function render_base( $settings ) {
		...
		echo '<div class="thim-countdown' . $class . '" id="coming-soon-counter' . $this->get_id() . '"></div>';
	...
	protected function render_base_pie( $settings ) {
		...
		if ( $settings['layout'] == 'pie' ) {
			echo '<div id="countdown' . esc_attr( $this->get_id() ) . '"  class="thim-countdown thim_countdown_pie style_white_wide"></div>';
			$theme = 'white-wide';
		} else {
