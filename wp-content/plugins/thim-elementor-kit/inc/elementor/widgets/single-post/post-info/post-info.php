...
class Thim_Ekit_Widget_Post_Info extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Thim Post Info', 'thim-elementor-kit' );
	}
	...
	protected function register_style_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'body:not(.rtl) {{WRAPPER}} .thim-ekit-single-post__info__content' => 'padding-left: {{SIZE}}{{UNIT}}',
					'body.rtl {{WRAPPER}} .thim-ekit-single-post__info__content' => 'padding-right: {{SIZE}}{{UNIT}}',
				),
			)
		);
	...
	protected function render_custom( $repeater_item ) {
		...
		if ( $show_icon === 'default' ) {
			...
			$icon = $repeater_item['selected_icon']['value'];
		}
