...
abstract class Thim_Ekits_Course_Base extends Widget_Base {
	...
	protected function render_thumbnail( $settings, $course ) {
		...
		if ( empty( $thumbnail_html ) ) {
			$thumbnail_html = LP()->image( 'no-image.png' );
		...
		<div class="thim-ekits-course__thumbnail">
			<a href="<?php the_permalink(); ?>" <?php Utils::print_unescaped_internal_string( $attributes_html ); ?> class="course-thumbnail">
	...
	protected function render_title( $settings, $item ) {
		...
		<<?php Utils::print_validated_html_tag( $item['title_tag'] ); ?> class="thim-ekits-course__title">
			<a href="<?php the_permalink(); ?>" <?php Utils::print_unescaped_internal_string( $attributes_html ); ?>>
				<?php the_title(); ?>
			</a>
	...
	protected function render_price( $course, $tag_html = 'div' ) {
	...
	protected function render_meta_data( $settings, $item, $course ) {
		...
		?>
		<div class="thim-ekits-course__meta elementor-repeater-item-<?php echo esc_attr( $item['_id'] ); ?><?php echo ' m-psi-' . esc_attr( $item['meta_data_display'] ); ?>">
			<?php
			if ( in_array( 'duration', $meta_data ) ) {
				$this->render_duration( $item );
			}
			if ( in_array( 'level', $meta_data ) ) {
				$this->render_level( $item );
			}
			if ( in_array( 'instructor', $meta_data ) ) {
				$this->render_instructor( $settings, $course );
			}
			if ( in_array( 'count_lesson', $meta_data ) ) {
				$this->render_count_lesson( $item, $course );
			}
			if ( in_array( 'count_quiz', $meta_data ) ) {
				$this->render_count_quiz( $item, $course );
			}
			if ( in_array( 'count_student', $meta_data ) ) {
				$this->render_count_student( $item, $course );
			}
			if ( in_array( 'price', $meta_data ) ) {
	...
	protected function render_duration( $settings ) {
		...
		<span class="thim-ekits-course__duration">
			...
			echo wp_kses_post( learn_press_get_post_translated_duration( get_the_ID(), esc_html__( 'Lifetime access', 'thim-elementor-kit' ) ) );
			?>
		</span>
		<?php
	}
	...
	protected function render_instructor( $settings, $course ) {
		...
		<span class="thim-ekits-course__instructor">
			<?php if ( $settings['instructor_avatar'] != 'none' ) : ?>
				<span class="thim-ekits-course__instructor__avatar"><?php echo wp_kses_post( $course->get_instructor()->get_profile_picture() ); ?></span>
			<?php endif; ?>
			<span class="thim-ekits-course__instructor__content"><?php echo wp_kses_post( $course->get_instructor_html() ); ?></span>
		</span>
		<?php
	}
	...
	protected function render_tags( $settings ) {
		$tags = get_the_tag_list( '', 'course_tag' );
	...
	protected function render_read_more( $settings, $text_read_more, $icon = false ) {
		$attributes_html = $this->get_optional_link_attributes_html( $settings );
		?>
		<a class="thim-ekits-course__read-more" href="<?php the_permalink(); ?>" <?php echo wp_kses_post( $attributes_html ); ?>>
			<?php
