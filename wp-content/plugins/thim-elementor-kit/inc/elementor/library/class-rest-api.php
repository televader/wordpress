	...
	public function import( $request ) {
		...
		try {
			...
			if ( class_exists( '\Thim_Product_Registration' ) ) {
				...
				$refresh_token = \Thim_Product_Registration::get_refresh_token();
			...
			if ( $api_body['code'] === 'no_code' ) {
				throw new \Exception( class_exists( '\Thim_Product_Registration' ) ? 'Please <a href="' . admin_url( '/admin.php?page=thim-dashboard' ) . '" target="_blank" rel="noopener">Login with Envato</a> to continue' : 'Please register your site to use this feature.' );
			}
