...
class Custom_Post_Type {
	...
	public function admin_columns_headers( array $columns ) : array {
		...
		$columns['type']      = esc_html__( 'Type', 'thim-elementor-kit' );
		$columns['shortcode'] = esc_html__( 'Shortcode', 'thim-elementor-kit' );
	...
	public function admin_columns_content( string $column_name, int $post_id ) {
		switch ( $column_name ) {
			case 'type':
				...
				$is_active = apply_filters( 'thim_ekit/modules/show_label_active', false, $post_id );

				$label = $is_active ? sprintf( '<span class="thim-ekit__table__badge">%s</span>', esc_html__( 'Active', 'thim-elementor-kit' ) ) : '';

				echo ! empty( $this->tabs[ $type ] ) ? sprintf( '<a href="%1s">%2s%3s</a>', esc_url( $this->tabs[ $type ]['url'] ), esc_html( $this->tabs[ $type ]['name'] ), $label ) : esc_html__( 'Unsupported', 'thim-elementor-kit' );
				break;
			...
			case 'shortcode':
				...
				<span class="thim-ekit-shortcode-column">
					<input type="text" onfocus="this.select();" readonly="readonly" value="<?php echo esc_attr( $shortcode ); ?>">
				</span>
				<?php
				break;
		}
